<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="UserFeed.aspx.cs" Inherits="Tag.User.UserFeed" %>
    ...
    <script type="text/javascript">
            ...
            if (checkCookie()) {
                $(document).ready(function () {
                    $.ajax({
                        ...
                        data: "{'premalinkId':'" + pid + "' ,'TagId':'<%= ViewState["TagId"].ToString() %>','vote':'" + vote + "'}",
        ...
        function pagging() {
            $(document).ready(function () {
                ...
                var makeurl = "../Pagging.aspx?Id=<%= UserId%>";
                <% if (ViewState["TagId"] != null)
                   {
                       Int64 res = 0;
                       Int64.TryParse(ViewState["TagId"].ToString(),out res);
                       %>
                    makeurl += "&TagId=<%= res %>";
                <% }
                   if (ViewState["EmoId"] != null)
                   {
                       Int64 res = 0;
                       Int64.TryParse(ViewState["EmoId"].ToString(), out res);
                %>
                    makeurl += "&EmoId=<%= res %>";
                <% } %>

                makeurl += "&flow=profile&PageNo=" + document.getElementById('<%= hdnpagenumber.ClientID %>').value;
                ...
                $.ajax({
                    ...
                    success: function (msg) {
                        ...
                        parent.window.resizeIframe(parent.document.getElementById('iframeuserfeed'));
                    ...
                    <tr>
                        <td style="width: 100%;">
                            <table style="width: 100%;">
                                <tr>
                                    <td style="vertical-align: top; width: 5%; text-align: right; padding-right: 10px;">
                                        <a href="javascript:window.parent.location='../Website/Website.aspx?WebsiteId=<%= item.WebsiteId %>'">
                            ...
                            <div style="text-align: left;">
                                <a style="color: black; font-weight: bold; text-decoration: none; padding-top: 10px;" href="javascript:window.parent.location='../Website/Website.aspx?WebsiteId=<%= item.WebsiteId %>'">
                                    <%= item.WebsiteName %>
                                </a>
                            </div>
                                                            <br />
